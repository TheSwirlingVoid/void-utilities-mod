...
import org.theswirlingvoid.VoidUtilities.PacketHandler;
...
public class BeaconntScreen extends ContainerScreen<BeaconntContainer> {
   private static final ResourceLocation BEACON_GUI_TEXTURES = new ResourceLocation("textures/gui/container/beacon.png");
   private BeaconntScreen.ConfirmButton beaconConfirmButton;
   private boolean buttonsNotDrawn;
   public BeaconntContainer container;
   private Effect field_214105_n;
   private Effect field_214106_o;
   private BlockPos pos;

   public BeaconntScreen(BeaconntContainer p_i51102_1_, PlayerInventory p_i51102_2_, ITextComponent p_i51102_3_) {
      super(p_i51102_1_, p_i51102_2_, p_i51102_3_);
      this.container=p_i51102_1_;
      this.pos=p_i51102_1_.pos;
      this.xSize = 230;
      this.ySize = 219;
      p_i51102_1_.addListener(new IContainerListener() {
         ...
         public void sendWindowProperty(Container containerIn, int varToUpdate, int newValue) {
            BeaconntScreen.this.field_214105_n = p_i51102_1_.func_216967_f();
            BeaconntScreen.this.field_214106_o = p_i51102_1_.func_216968_g();
            BeaconntScreen.this.buttonsNotDrawn = true;
//            BeaconntScreen.this.pos=((BeaconntContainer)containerIn).pos;
         }
      });
   }
   ...
   protected void init() {
      super.init();
      this.beaconConfirmButton = this.addButton(new BeaconntScreen.ConfirmButton(this.guiLeft + 164, this.guiTop + 107,this));
      this.addButton(new BeaconntScreen.CancelButton(this.guiLeft + 190, this.guiTop + 107));
   ...
   public void tick() {
      ...
      if (this.buttonsNotDrawn && i >= 0) {
         ...
         for(int j = 0; j <= 2; ++j) {
            int k = BeaconntTileEntity.EFFECTS_LIST[j].length;
            int l = k * 22 + (k - 1) * 2;

            for(int i1 = 0; i1 < k; ++i1) {
               Effect effect = BeaconntTileEntity.EFFECTS_LIST[j][i1];
               BeaconntScreen.PowerButton beaconscreen$powerbutton = new BeaconntScreen.PowerButton(this.guiLeft + 76 + i1 * 24 - l / 2, this.guiTop + 22 + j * 25, effect, true);
         ...
         int j1 = 3;
         int k1 = BeaconntTileEntity.EFFECTS_LIST[3].length + 1;
         int l1 = k1 * 22 + (k1 - 1) * 2;

         for(int i2 = 0; i2 < k1 - 1; ++i2) {
            Effect effect1 = BeaconntTileEntity.EFFECTS_LIST[3][i2];
            BeaconntScreen.PowerButton beaconscreen$powerbutton2 = new BeaconntScreen.PowerButton(this.guiLeft + 167 + i2 * 24 - l1 / 2, this.guiTop + 47, effect1, false);
         ...
         if (this.field_214105_n != null) {
            BeaconntScreen.PowerButton beaconscreen$powerbutton1 = new BeaconntScreen.PowerButton(this.guiLeft + 167 + (k1 - 1) * 24 - l1 / 2, this.guiTop + 47, this.field_214105_n, false);
   ...
   abstract static class Button extends AbstractButton {
      ...
      public void renderButton(int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_) {
         Minecraft.getInstance().getTextureManager().bindTexture(BeaconntScreen.BEACON_GUI_TEXTURES);
         GlStateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
         int i = 219;
         int j = 0;
         if (!this.active) {
   ...
   @OnlyIn(Dist.CLIENT)
   class CancelButton extends BeaconntScreen.SpriteButton {
      public CancelButton(int p_i50829_2_, int p_i50829_3_) {
         super(p_i50829_2_, p_i50829_3_, 112, 220);
      }

      public void onPress() {
         BeaconntScreen.this.minecraft.player.connection.sendPacket(new CCloseWindowPacket(BeaconntScreen.this.minecraft.player.openContainer.windowId));
         BeaconntScreen.this.minecraft.displayGuiScreen((Screen)null);
      }

      public void renderToolTip(int p_renderToolTip_1_, int p_renderToolTip_2_) {
         BeaconntScreen.this.renderTooltip(I18n.format("gui.cancel"), p_renderToolTip_1_, p_renderToolTip_2_);
      }
   }

   @OnlyIn(Dist.CLIENT)
   class ConfirmButton extends BeaconntScreen.SpriteButton {
	   public BeaconntScreen screen;
      public ConfirmButton(int p_i50828_2_, int p_i50828_3_,BeaconntScreen screenIn) {
         super(p_i50828_2_, p_i50828_3_, 90, 220);
         screen=screenIn;
      }

      public void onPress() {
//    	  BeaconntScreen.this.getContainer().putStackInSlot(0, ItemStack.EMPTY);
//    	  BeaconntScreen.this.getContainer().detectAndSendChanges();
         PacketHandler.sendBeaconntPacket(getID(BeaconntScreen.this.field_214105_n), getID(BeaconntScreen.this.field_214106_o),BeaconntScreen.this.pos);
//         System.out.println(BeaconntScreen.this.pos.getX());
         BeaconntScreen.this.minecraft.player.connection.sendPacket(new CCloseWindowPacket(BeaconntScreen.this.minecraft.player.openContainer.windowId));
         BeaconntScreen.this.minecraft.displayGuiScreen((Screen)null);
      }
public int getID(Effect e) {
	if (e==null) {
		return -1;
	}
	return Effect.getId(e);
}
      public void renderToolTip(int p_renderToolTip_1_, int p_renderToolTip_2_) {
         BeaconntScreen.this.renderTooltip(I18n.format("gui.done"), p_renderToolTip_1_, p_renderToolTip_2_);
      }
   }

   @OnlyIn(Dist.CLIENT)
   class PowerButton extends BeaconntScreen.Button {
      ...
      public void onPress() {
         if (!this.isSelected()) {
            if (this.field_212947_d) {
               BeaconntScreen.this.field_214105_n = this.effect;
            } else {
               BeaconntScreen.this.field_214106_o = this.effect;
            }

            BeaconntScreen.this.buttons.clear();
            BeaconntScreen.this.children.clear();
            BeaconntScreen.this.init();
            BeaconntScreen.this.tick();
         }
      }

      public void renderToolTip(int p_renderToolTip_1_, int p_renderToolTip_2_) {
         String s = I18n.format(this.effect.getName());
         if (!this.field_212947_d && this.effect != Effects.GLOWING) {
            s = s + " II";
         }

         BeaconntScreen.this.renderTooltip(s, p_renderToolTip_1_, p_renderToolTip_2_);
      }
   ...
   @OnlyIn(Dist.CLIENT)
   abstract static class SpriteButton extends BeaconntScreen.Button {
