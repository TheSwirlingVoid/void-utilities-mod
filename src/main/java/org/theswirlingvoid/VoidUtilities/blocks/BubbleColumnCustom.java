...
public class BubbleColumnCustom extends Block implements IBucketPickupHandler {
   public static final BooleanProperty DRAG = BlockStateProperties.DRAG;

   public BubbleColumnCustom(Block.Properties properties) {
      super(properties);
   ...
   public static void placeBubbleColumn(IWorld p_203159_0_, BlockPos p_203159_1_, boolean drag) {
      if (canHoldBubbleColumn(p_203159_0_, p_203159_1_)) {
         p_203159_0_.setBlockState(p_203159_1_, ModBlocks.BUBBLE_COLUMN.getDefaultState().with(DRAG, Boolean.valueOf(drag)), 2);
      }
   ...
   public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      if (!stateIn.isValidPosition(worldIn, currentPos)) {
         ...
         if (facing == Direction.DOWN) {
            worldIn.setBlockState(currentPos, ModBlocks.BUBBLE_COLUMN.getDefaultState().with(DRAG, Boolean.valueOf(getDrag(worldIn, facingPos))), 2);
         } else if (facing == Direction.UP && facingState.getBlock() != ModBlocks.BUBBLE_COLUMN && canHoldBubbleColumn(worldIn, facingPos)) {
   ...
   public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
      ...
      return block == ModBlocks.BUBBLE_COLUMN || block == Blocks.MAGMA_BLOCK || block == Blocks.SOUL_SAND||block==ModBlocks.soulsandnt;
   }
